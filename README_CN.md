...
基于 **Go** 的轻量 **SSH 服务端**，仅支持 **远程 TCP 转发 (`-R`)**（不提供本地转发 `-L` / `direct-tcpip`，避免服务端代拨任意目标），并带 **Web 管理界面**：维护 SSH 登录账号/密码，以及每个客户端允许使用的 **远程转发端口**，并在页面上查看当前活跃的 `-R` 监听。

## 功能概览

- **SSH**：密码认证（bcrypt 存储）；主机密钥首次运行自动生成（Ed25519，保存在数据目录）。
- **远程转发 (`-R`)**：仅在 Web 中为该用户登记的端口上监听；未配置端口则拒绝 `tcpip-forward`。
- **本地转发 (`-L`)**：**已关闭**（拒绝 `direct-tcpip` 通道）。
...
`ssh -L` 对本服务**不可用**（服务端会拒绝，仅保留 `-R` 以降低滥用风险）。
